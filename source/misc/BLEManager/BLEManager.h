...
@interface Manager : NSObject <CBCentralManagerDelegate, CBPeripheralDelegate>{
    ...
    t_atom manager_input[MAX_PERIPHERAL][MAX_PIN]; //stores all pin values (sent by peripheral)
    bool manager_output[MAX_PERIPHERAL][MAX_OUTPUT]; //stores all user values (to be sent to peripheral)
    bool manager_connected[MAX_PERIPHERAL];
    NSMutableDictionary * manager_uuid_to_index;
    t_atom manager_return[MAX_PIN+1]; // final array to output
    bool manager_set[MAX_OUTPUT]; // final array to send to peripheral
